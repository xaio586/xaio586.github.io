...
function resourceTextMarkup(resource,page,toc){if(toc)return `<div class="resource-page-text">${esc(page.text.replace(/\n{2,}/g,'\n'))}</div>`;return `<div class="resource-page-text resource-sentences">${sentenceChunks(page.text).map((sentence,index)=>{const key=`${resource.id}:${page.page_number}:${index}`;const heading=sentence.length<105&&(/^(?:IELTS (?:Writing Task 2|Advice):|Note:|Task:)/i.test(sentence)||/[?:：]$/.test(sentence));return `<button type="button" class="reading-line${resourceHighlights[key]?' is-highlighted':''}${heading?' is-heading':''}" data-highlight-key="${esc(key)}">${esc(sentence)}</button>`}).join('')}</div>`}
...
async function openSpeakingTopic(id,trigger=null){
  ...
  $('#detail-drawer').classList.remove('writing-detail','resource-detail');
  const done=completedCount(g);
...
async function openResource(id){const d=await api('/api/resources/'+id);const drawer=$('#detail-drawer');drawer.classList.remove('writing-detail');drawer.classList.add('resource-detail');const images=d.illustrations||[];const hero=images[0]||d.cover_url;const inks=d.palette?.inks||['#315cc1','#c65f38'];window.setIeltsBackgroundTheme?.(inks[0],inks[1]);const pages=(d.pages||[]).filter(page=>String(page.text||'').trim());const body=pages.map((page,index)=>{const toc=(page.text.match(/· p\./g)||[]).length>=3;const interlude=index>0&&index%2===0&&images.length>1&&!toc?`<figure class="resource-interlude"><img src="${esc(images[1+Math.floor(index/2-1)%(images.length-1)])}" alt="" loading="lazy" decoding="async"></figure>`:'';const label=pages.length>1?`<span>PAGE ${String(page.page_number).padStart(2,'0')}</span>`:'';return `${interlude}<section class="resource-page${toc?' is-toc':''}" id="resource-page-${page.page_number}">${label}${resourceTextMarkup(d,page,toc)}</section>`}).join('');setDetailContent(`<article class="resource-reader">${hero?`<figure class="resource-reader-hero"><img src="${esc(hero)}" alt="" fetchpriority="high" decoding="async"></figure>`:''}<header><span class="detail-label">${esc(d.metadata?.category_label||'写作资料')}</span><h2>${esc(d.title)}</h2><p>${pages.length} 页 · 已转换为 App 内文字阅读</p></header><div class="resource-tools">${resourceOutlineMarkup(d)}<button type="button" id="resource-marker" class="resource-marker" aria-pressed="false">记号笔</button></div>${body||'<div class="empty-state"><strong>这份资料暂时没有可提取的文字</strong></div>'}</article>`);openDrawer();bindResourceTools(d)}
