...
async function loadTopics(){const topics=await api('/api/topics');window.topicCache=topics;$('#topic-grid').innerHTML=topics.map((t,i)=>{const inks=t.palette?.inks||['#2148B8','#C65F38'];const visual=t.cover_url?`<span class="category-visual"><img src="${esc(t.cover_url)}" alt="" loading="lazy"></span>`:'<span class="category-visual category-placeholder"></span>';return `<button class="topic-card" data-topic="${t.key}" style="--cover-a:${esc(inks[0])};--cover-b:${esc(inks[1])}">${visual}<span class="topic-card-content"><span class="topic-icon">${String(i+1).padStart(2,'0')}</span><strong>${t.count}</strong><h2>${esc(t.name)}</h2><p>${esc(t.description)}</p></span></button>`}).join('');$$('.topic-card').forEach(card=>card.onclick=()=>showCategory(card.dataset.topic))}
function coverVisual(g){const keys=(g.keywords||[]).slice(0,3);const inks=g.palette?.inks||['#2148B8','#C65F38'];const image=g.cover_url?`<img src="${esc(g.cover_url)}" alt="" loading="lazy" draggable="false">`:'<span class="cover-plate" aria-hidden="true"></span><span class="cover-pending">封面生成中</span>';return `<div class="topic-cover ${g.cover_url?'has-cover':'is-placeholder'}" style="--cover-a:${esc(inks[0])};--cover-b:${esc(inks[1])}">${image}<div class="cover-keywords" aria-label="主题关键词：${keys.map(esc).join('、')}">${keys.map((k,i)=>`<span class="cover-keyword k${i+1}">${esc(k)}</span>`).join('')}</div><span class="cover-number" aria-hidden="true">${g.family==='p1'?'P1':'P2·3'}</span></div>`}
function groupCard(g){
  ...
  return `<article class="speaking-topic-card ${done===g.question_count?'topic-complete':''}" style="--cover-a:${esc(inks[0])};--cover-b:${esc(inks[1])}" data-group-id="${g.id}" tabindex="0">${coverVisual(g)}<div class="topic-card-copy"><div class="topic-card-top"><div>${status}<h2>${esc(g.title)}</h2></div><span class="season">2026年9–12月</span></div><p>${count} · 已练 <b>${done}/${g.question_count}</b></p><div class="progress-track" aria-label="练习进度 ${percent}%"><span style="width:${percent}%"></span></div><div class="topic-preview" aria-hidden="true"><div class="preview-label"><span>题目速览</span><span>PREVIEW / ${g.question_count}</span></div><ol>${previews}</ol></div><div class="topic-card-foot"><span>${g.family==='p1'?'Part 1':'P2 · P3'}</span><span>${esc(topicName(g.topic))}</span><button>去练习 →</button></div></div></article>`;
}
...
function bindPhotoDeck(box,groups,navigation,toolbar){
  ...
  const image=g=>g.cover_url?`url("${String(g.cover_url).replace(/["\\]/g,'')}")`:'none';
  ...
  const replaceCard=(card,g)=>{
    const template=document.createElement('template');template.innerHTML=groupCard(g);
