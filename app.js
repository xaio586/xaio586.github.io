...
function coverVisual(g,loading='lazy'){const keys=(g.keywords||[]).slice(0,3);const inks=g.palette?.inks||['#2148B8','#C65F38'];const image=g.cover_url?`<img src="${esc(g.cover_url)}" alt="" loading="${loading}" decoding="async" draggable="false">`:'<span class="cover-plate" aria-hidden="true"></span><span class="cover-pending">封面生成中</span>';return `<div class="topic-cover ${g.cover_url?'has-cover':'is-placeholder'}" style="--cover-a:${esc(inks[0])};--cover-b:${esc(inks[1])}">${image}<div class="cover-keywords" aria-label="主题关键词：${keys.map(esc).join('、')}">${keys.map((k,i)=>`<span class="cover-keyword k${i+1}">${esc(k)}</span>`).join('')}</div><span class="cover-number" aria-hidden="true">${g.family==='p1'?'P1':'P2·3'}</span></div>`}
